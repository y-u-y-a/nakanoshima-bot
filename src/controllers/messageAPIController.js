...
class MessageAPIController {
    ...
    async messageTextHandler(messageText) {
        switch (true) {
            case /ミツル|みつる|充/.test(messageText): {
                return { type: 'text', text: 'リア充のみつるだよ〜、部屋番号は410だあよ。' }
            }
    ...
    async postbackDataHandler(postbackData) {
        // switch文の省略形
        const events = {
            eventsIndex: { type: 'text', text: await eventsController.index() },
            eventsCreate: { type: 'text', text: await eventsController.create() },
        }
        return events[postbackData] ? events[postbackData] : defaultResponse
    }
}
